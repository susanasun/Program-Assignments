...
total_price(groceries["Chicken"], groceries["Cheese"])
...
def price_difference(item1, item2):
    ...
    print("The difference between " + str(item1) + " and " + str(item2), " is " + abs(round(difference, 2)))
...
def Mix_Numbers(Name1, Name2):
    ...
    PHONE_numbers["Jeff"] = Updated_shoe1

    PHONE_numbers["Jones"] = Updated_shoe2
...
for i in range(length):
    length = len(listPlayers)
    randomNumber = randomNumber.randint(0,length-1)
    ...
    for j in range(size-1):
        if listNumbers[j] < listNumbers[j+1]:
            listNumbers.append(listNumbers[j])
            listNumbers.pop(j)
